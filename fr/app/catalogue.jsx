...
function FilterSidebar({ filters, setFilters, items }) {
  const Tags = ["Socle","Cardio","Force","Alimentation","Cerveau","Esprit","Chaleur","Frontière","Sénolytique","Sur ordonnance","Sans ordonnance","NAD+","Mitophagie","Autophagie"];
  const toggle = (k, v) => {
    const cur = filters[k] || [];
  ...
  return (
    <aside style={{ width: 240, padding: "8px 16px 24px", borderRight: "1px solid var(--rule)", background: "var(--paper)", position: "sticky", top: 60, height: "calc(100vh - 60px)", overflowY: "auto", flexShrink: 0 }}>
      <div className="filter-block" style={{ borderBottom: 0, paddingBottom: 0 }}>
        <div className="filter-h"><span>Filtres</span><button className="btn ghost" style={{ padding: "0 4px", fontSize: 10 }} onClick={() => setFilters({})}>Réinitialiser</button></div>
      </div>
      <div className="filter-block">
        <div className="filter-h">Niveau</div>
      ...
      <div className="filter-block">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[["lifestyle","Mode de vie"],["supplement","Complément"],["medication","Médicament"]].map(([k,l]) => (
      ...
      <div className="filter-block">
        <div className="filter-h">Niveau de preuve</div>
      ...
      <div className="filter-block">
        <div className="filter-h">Préoccupations</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div className="filter-block">
        <div className="filter-h">Étiquettes</div>
...
function CatalogueView({ items, viz, onPick }) {
  ...
  const activeChips = [
    ...
    ...(filters.ev || []).map(t => ({ k: "ev", v: t, l: `Preuve ${t}` })),
    ...
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "100%" }}>
      ...
      <div style={{ padding: "24px 32px 80px", minWidth: 0 }}>
        <header style={{ marginBottom: 20 }}>
          <div className="kicker" style={{ marginBottom: 6 }}>Bibliothèque</div>
          <h1 style={{ fontFamily: "var(--display)", fontStyle: "var(--display-italic)", fontWeight: "var(--display-w)", fontSize: 32, margin: "0 0 4px", letterSpacing: "-0.02em" }}>Catalogue</h1>
          <div style={{ fontSize: 13.5, color: "var(--ink-2)" }}>{filtered.length} sur {items.length} interventions · évaluées à partir de 412 études humaines</div>
        </header>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ display: "inline-flex", border: "1px solid var(--rule)", borderRadius: 8, overflow: "hidden", background: "var(--paper)" }}>
            {["grid","list","scatter"].map(l => (
              <button key={l} className="btn ghost" style={{ borderRadius: 0, padding: "7px 12px", fontSize: 12, background: layout === l ? "var(--bg-2)" : "transparent" }} onClick={() => setLayout(l)}>
                {{grid:"Grille",list:"Liste",scatter:"Nuage"}[l]}
              </button>
            ))}
          </div>
          {layout === "grid" && (
            <div style={{ display: "inline-flex", border: "1px solid var(--rule)", borderRadius: 8, overflow: "hidden", background: "var(--paper)" }}>
              {["comfy","dense"].map(d => (
                <button key={d} className="btn ghost" style={{ borderRadius: 0, padding: "7px 12px", fontSize: 12, background: density === d ? "var(--bg-2)" : "transparent" }} onClick={() => setDensity(d)}>{{comfy:"Confortable",dense:"Dense"}[d]}</button>
              ))}
            </div>
          )}
          <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: "7px 10px", border: "1px solid var(--rule)", borderRadius: 8, background: "var(--paper)", fontSize: 12 }}>
            <option value="effect">Trier : Efficacité ↓</option>
            <option value="certainty">Trier : Certitude ↓</option>
            <option value="combined">Trier : Combiné ↓</option>
        ...
        {layout === "list" && (
          <div className="card" style={{ padding: 0 }}>
            <div className="list-row" style={{ gridTemplateColumns: "auto 1.6fr 1fr 110px 90px 70px 24px", padding: "10px 18px", background: "var(--paper-2)", borderRadius: "14px 14px 0 0", cursor: "default" }}>
              ...
              <span className="mono" style={{ color: "var(--ink-3)" }}>Cibles</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Score</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Preuve</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Coût</span>
              <span></span>
            </div>
            {filtered.map(item => (
        ...
        {layout === "scatter" && (
          <div className="card card-pad" style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 24, alignItems: "center" }}>
            ...
            <div>
              <div className="kicker" style={{ marginBottom: 8 }}>Lecture du graphique</div>
              <p style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.55, marginTop: 0 }}>
                Chaque point représente une intervention. En haut à droite : « effet important, grande certitude » — c'est là que se trouvent le sommeil, l'exercice, les statines. En bas à droite : la frontière de la longévité — peut-être considérable, peut-être rien.
              </p>
              <hr className="rule" style={{ margin: "12px 0" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="lifestyle" /> Mode de vie</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="supplement" /> Complément</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="medication" /> Médicament</div>
              </div>
            </div>
          </div>
        )}
