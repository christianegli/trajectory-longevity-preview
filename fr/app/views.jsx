...
const MONETIZATION_PLANS = {
  plus: {
    key: "plus",
    name: "Foundation stack interest",
    price: "Evidence-gated",
    eyebrow: "Optional interest capture",
    promise: "Content, hub, packets, watchlists, and methodology stay free. Register interest only if you want evidence-aligned stack fulfillment later.",
  },
  clinic: {
    key: "clinic",
    name: "Review-ready stack interest",
    price: "Evidence-gated",
    eyebrow: "Higher-caution review track",
    promise: "For people with more context to review: keep the free packet and watchlist, then register interest in a future safety-reviewed stack path.",
  },
};

function monetizationPlanForAssessment(ans = {}) {
  const concerns = answerList(ans, "concerns");
  const signals = answerList(ans, "signals");
  const highTouch =
    signals.length >= 2 ||
    concerns.includes("Heart disease") ||
    concerns.includes("Menopause transition") ||
    ans.age === "65+";
  return highTouch ? MONETIZATION_PLANS.clinic : MONETIZATION_PLANS.plus;
}

function buildStackInterestSnapshot(ans = {}, plan = null, recommendation = null, source = "assessment") {
  const selectedPlan = monetizationPlanForAssessment(ans);
  ...
  return {
    ...
    stackId: rec.stack?.id || null,
    stackName: rec.stack?.name || null,
    ingredients: rec.stack?.ingredients || [],
    reason: rec.reason || "",
    commerceState: rec.commerceState || "evidence_gated",
    assessmentDigest: {
...
function stackInterestReason(ans = {}) {
  const concerns = answerList(ans, "concerns");
  const signals = answerList(ans, "signals");
  if (signals.length) return "You have measured risk signals, so Trajectory keeps the core plan free and treats any future stack as evidence-gated until safety, fulfillment, and review standards are approved.";
  if (concerns.length) return `Your plan is focused on ${concerns.slice(0, 2).join(" and ")}. The free hub should keep that path current; optional stack commerce should only appear when evidence and fit justify it.`;
  return "The free hub remembers the plan, flags evidence changes, and prevents stack drift. This card only captures interest in future evidence-aligned stack fulfillment.";
}

function PlanOffer({ ans = {}, account = {}, onAccountPatch, source = "assessment" }) {
  const plan = monetizationPlanForAssessment(ans);
  ...
  const captureInterest = async () => {
    ...
    try {
      const stackInterest = {
        ...(account.stackInterest || buildStackInterestSnapshot(ans, null, null, source)),
        status: "registered",
        source,
        planKey: plan.key,
        planName: plan.name,
      ...
      await trajectoryApi("/api/billing/interest", {
        method: "POST",
        body: JSON.stringify({
          email,
          name: account.name || "",
          ...
          planKey: plan.key,
          intentScore: plan.key === "clinic" ? 95 : 75,
          newsletters: account.newsletters || ["weekly", "product"],
          assessment: ans,
          path: window.location.pathname + window.location.hash,
          route: window.location.hash || "#/assessment",
          tags: ["assessment-plan"],
        }),
      });
      window.trackTrajectoryEvent && window.trackTrajectoryEvent("billing_interest", { source, planKey: plan.key });
      setStatus("Saved. This is now visible in CRM.");
    } catch (e) {
  ...
  return (
    <section className="plan-offer">
      <div>
        ...
        <h3 className="display">{plan.name}</h3>
      ...
      <div className="plan-offer-card">
        ...
        <div className="plan-email-row">
          ...
          <button className="btn primary" onClick={captureInterest}>Register interest</button>
        </div>
        <small>No charge today. This reserves demand for the evidence-aligned stack path we are preparing.</small>
...
function MembershipPanel({ user, setUser }) {
  const [status, setStatus] = vS("");
  const subscription = user?.subscription;
  const startTrial = async (planKey) => {
    setStatus(`Starting ${planKey} trial...`);
    try {
      const data = await trajectoryApi("/api/billing/trial", {
        method: "POST",
        body: JSON.stringify({
          planKey,
          language: trajectoryLanguage(),
          path: window.location.pathname + window.location.hash,
          route: window.location.hash || "#/account",
        }),
      });
      setUser(data.user);
      window.trackTrajectoryEvent && window.trackTrajectoryEvent("trial_started", { planKey });
      setStatus(`${planKey === "clinic" ? "Review-ready stack" : "Foundation stack"} trial active`);
    } catch (e) {
      setStatus(e.message || "Could not start trial");
    }
  };
  if (!user) {
    return (
      <section className="membership-panel empty">
        <div>
          <div className="kicker clay"><span className="bullet" />Membership</div>
          <h3 className="display">Create an account to test future commerce flows.</h3>
          <p>Demand capture works without login; internal trials require an account so the platform can test future stack entitlements without charging users.</p>
        </div>
      </section>
    );
  }
  return (
    <section className="membership-panel">
      <div>
        <div className="kicker clay"><span className="bullet" />Membership</div>
        <h3 className="display">{subscription ? `${subscription.plan_key} interest track active` : "No stack interest track active"}</h3>
        <p>
          {subscription
            ? `${subscription.status} from ${subscription.source}. Trial ends ${subscription.trial_ends_at ? new Date(subscription.trial_ends_at).toLocaleDateString() : "when commerce approval says so"}.`
            : "Start a local trial to test entitlement-aware stack surfaces before any checkout is connected."}
        </p>
      </div>
      <div className="membership-actions">
        <button className="btn secondary" onClick={()=>startTrial("plus")}>Test foundation stack interest</button>
        <button className="btn primary" onClick={()=>startTrial("clinic")}>Test review-ready interest</button>
        {status && <div className="save-toast">{status}</div>}
      </div>
    </section>
  );
    ...
    <div className="path-panel">
      ...
      {recommendation.stack ? (
        <>
          ...
          <small>Checkout remains gated until fulfillment, safety review, and evidence-fit standards are ready. Food, training, sleep, screening, and medications may matter more; review contraindications before any supplement decision.</small>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
            <button className="btn secondary" onClick={()=>setRoute("assessment")}>Update assessment</button>
            <a className="btn ghost" href="stacks/">View stack methodology →</a>
          </div>
        </>
      ) : (
        <>
          ...
          <p>The hub stays useful while commerce is still evidence-gated: keep following the free action path, watchlist, and review packet.</p>
          <button className="btn secondary" onClick={()=>setRoute("assessment")}>Update assessment</button>
        </>
      )}
    </div>
    ...
    <section className="path-hub">
      <div className="path-hub-head">
        <div>
          ...
          <p>Track the few moves Trajectory thinks matter now. The free hub separates actions, context, and optional stack support so commerce is downstream of the evidence path.</p>
        </div>
...
function buildReviewPacket(items, ans = {}) {
  ...
  return {
    ...
    paidPlan: selectedPlan.key,
    paidPlanName: selectedPlan.name,
    answers: ans,
...
function packetMarkdown(packet) {
  const lines = [
    ...
    `Optional stack interest: ${packet.paidPlanName}`,
    ``,
    `## Context`,
    ...
    <section className="review-packet">
      <div className="review-packet-head">
        <div>
          ...
          <p>Generated from the assessment, ordered by evidence and context. The packet is free, saveable, copyable, and designed for review before any optional stack commerce.</p>
        </div>
...
function AssessmentResults({ items, ans, account, onPick, setRoute, onSaveAccount, onAccountPatch }) {
  ...
  vE(() => {
    ...
    const same = prev.status === snapshot.status && prev.planKey === snapshot.planKey && prev.stackId === snapshot.stackId && prev.reason === snapshot.reason;
  ...
  const sectionCopy = {
    lifestyle: "Behavior and routine levers first: sleep, training, alcohol, smoking, light, stress, and connection before products.",
    ...
    optionalSupplement: "Optional supplement support appears only after lifestyle, medical discussion, and food-first changes are separated. Commerce stays evidence-gated.",
  };
  const resultText = [
    ...
    signals.length ? "Known risk signals were moved up the queue without turning them into a purchase recommendation." : "",
    stackRecommendation.stack ? `One optional ${stackRecommendation.stack.name} may fit, evidence-gated.` : "No stack is recommended by default from this assessment.",
  ].filter(Boolean).join(" ");
  ...
  return (
    <div className="assessment-results">
      <div className="card deep card-pad assessment-result-hero">
        ...
        <h2 className="display">Separate the levers before considering a stack.</h2>
      ...
      <section className="card card-pad" style={{ marginBottom: 22 }}>
        <div className="kicker" style={{ marginBottom: 10 }}><span className="bullet" />Optional supplement support</div>
        ...
        <div className="card card-pad-sm" style={{ marginTop: 12 }}>
          <div className="kicker">Evidence-aligned stack interest</div>
          {stackRecommendation.stack ? (
            <div>
              <h3 className="display" style={{ fontSize: 24, margin: "8px 0" }}>{stackRecommendation.stack.name}</h3>
              <p style={{ margin: "0 0 8px", color: "var(--ink-2)", fontSize: 14, lineHeight: 1.6 }}>{stackRecommendation.stack.positioning}</p>
              <p style={{ margin: "0 0 8px", color: "var(--ink-2)", fontSize: 14, lineHeight: 1.6 }}>{stackRecommendation.reason}</p>
              <div className="assessment-mini-grid">
                {stackIngredientItems.map(item => (
                  <div key={item.slug} className="assessment-already-card">
                    <span className="assessment-check">•</span>
                    <button onClick={()=>onPick(item.slug)}>{item.name}</button>
                  </div>
                ))}
              </div>
              <small>Commerce state: {stackRecommendation.commerceState}. Stack fit is saved automatically; checkout is gated until the product, fulfillment, and safety-review path is ready.</small>
            </div>
          ) : (
            <div>
              <h3 className="display" style={{ fontSize: 24, margin: "8px 0" }}>No stack recommended</h3>
              <p style={{ margin: "0 0 8px", color: "var(--ink-2)", fontSize: 14, lineHeight: 1.6 }}>{stackRecommendation.reason}</p>
              <small>Commerce state remains {stackRecommendation.commerceState}; free evidence and review tools stay primary.</small>
            </div>
          )}
        </div>
      </section>
      ...
      {stackRecommendation.stack && <PlanOffer ans={ans} account={account} onAccountPatch={onAccountPatch} source="assessment_results" />}
...
function AccountView({ setRoute }) {
  ...
  const submitAuth = async () => {
    ...
    setAccount(next);
    saveTrajectoryAccount(next);
    await trajectoryApi("/api/account/profile", { method: "PUT", body: JSON.stringify({ name: next.name, newsletters: next.newsletters || [], assessment: next.assessment || {}, plan: next.plan || {}, watchlist: next.watchlist || [], progress: next.progress || {}, stackInterest: next.stackInterest || {} }) });
  ...
  return (
    <div className="page account-page">
      ...
      <PlanOffer ans={account.assessment || {}} account={account} onAccountPatch={syncLocal} source="account" />
      <MembershipPanel user={user} setUser={setUser} />
...
function PlanCard({ plan, account, setAccount, emphasis }) {
  const [email, setEmail] = vS(account.email || "");
  const [status, setStatus] = vS("");
  vE(() => { if (account.email) setEmail(account.email); }, [account.email]);
  ...
  const capture = async () => {
    ...
    setStatus("Saving interest...");
    try {
      const next = { ...account, email, updatedAt: new Date().toISOString() };
      setAccount(next);
      saveTrajectoryAccount(next);
      await trajectoryApi("/api/billing/interest", {
        method: "POST",
        body: JSON.stringify({
          ...
          path: window.location.pathname + window.location.hash,
          route: window.location.hash || "#/plans",
          tags: ["plans-page"],
        }),
      });
      window.trackTrajectoryEvent && window.trackTrajectoryEvent("billing_interest", { source: "plans", planKey: plan.key });
      setStatus("Saved. We can now see demand for this plan.");
    } catch (e) {
      setStatus(e.message || "Could not save");
    }
  };
  return (
    <section className={`plan-card ${emphasis ? "emphasis" : ""}`}>
      <div>
        <div className="kicker clay"><span className="bullet" />{plan.eyebrow}</div>
      ...
      {isFree ? (
        <button className="btn secondary" onClick={() => window.location.hash = "#/catalogue"}>Open catalogue</button>
      ) : (
        <div className="plan-card-action">
          <input className="text-input" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" />
          <button className="btn primary" onClick={capture}>Register interest</button>
          {status && <div className="save-toast">{status}</div>}
        </div>
      )}
    </section>
...
function PlansView({ setRoute }) {
  const [account, setAccount] = vS(() => loadTrajectoryAccount());
  const plans = [
    {
      key: "free",
      eyebrow: "Free trust hub",
      name: "Trajectory",
      price: "EUR 0",
      promise: "The open research surface: catalogue, notes, methodology, assessment, personal hub, review packet, and watchlist.",
      features: ["Evidence-graded catalogue", "Free assessment and personal hub", "Free review packet and watchlist"],
    },
    {
      ...MONETIZATION_PLANS.plus,
      features: ["Interest-only signal", "Checkout gated until launch standards are met", "Future stack fulfillment only after approval"],
    },
    {
      ...MONETIZATION_PLANS.clinic,
      features: ["Higher-caution interest signal", "Free review packet remains available", "Future safety-reviewed path only after approval"],
    },
  ];
  return (
    <div className="page plans-page">
      <header className="plans-hero">
        <div>
          <div className="kicker clay"><span className="bullet" />Free hub + stack interest</div>
          <h1 className="display">Content stays free. Trust is the moat.</h1>
          <p>The free product tells you what is real, remembers your context, watches evidence changes, and keeps the plan from drifting. Commerce is only an evidence-gated signal for future evidence-aligned stacks.</p>
        </div>
        <button className="btn primary" onClick={() => setRoute("assessment")}>Start assessment</button>
      </header>
      <div className="plans-grid">
        {plans.map(plan => <PlanCard key={plan.key} plan={plan} account={account} setAccount={setAccount} emphasis={plan.key === "plus"} />)}
      </div>
      <section className="plans-note">
        <div className="kicker"><span className="bullet" />Shipping order</div>
        <p>First keep the hub free and capture stack interest only. Do not add checkout, account entitlements, or restricted packets until approval. No fake scarcity, no medical promises, no paywall around basic evidence.</p>
      </section>
    </div>
  );
}
...
function NotesView({ onPickPost }) {
  ...
  return (
    <div className="page notes-index">
      <header className="notes-index-hero newspaper-hero">
        <div>
          <div className="kicker"><span className="bullet" />The Trajectory Review</div>
          <h1 className="display">Field notes, edited like a Sunday paper.</h1>
          <p>
            Curated essays, protocol memos, and evidence updates for readers who want the signal without pretending Trajectory will never become a business.
          </p>
        </div>
        <div className="newspaper-nameplate" aria-hidden="true">
          <span>Evidence desk</span>
          <strong>{new Date().toLocaleDateString(undefined, { month: "short", day: "numeric" })}</strong>
          <small>{posts.length} filed notes</small>
        </div>
      </header>

      <nav className="notes-filter newspaper-filter">
      ...
      {featured && (() => {
        const visual = noteVisual(featured, 0);
        return (
        <a className="notes-feature newspaper-lead" href={`#/notes/${featured.slug}`} onClick={(e)=>{ e.preventDefault(); onPickPost && onPickPost(featured.slug); }}>
          <div className="newspaper-illustration lead" style={{ "--note-ink": visual.ink, "--note-paper": visual.paper }}>
            <span>{visual.n}</span>
            <i />
            <b />
          </div>
          <div>
            <div className="kicker"><span className="bullet" />Lead story · {featured.date}{featured.tag ? ` · ${featured.tag}` : ""}</div>
            <h2 className="display">{featured.title}</h2>
            <p>{featured.lede}</p>
          </div>
          <span className="notes-feature-arrow">Read</span>
        </a>
        );
      })()}
      ...
      <div className="notes-grid">
        {rest.map((p, index) => {
          const visual = noteVisual(p, index + 1);
          return (
          <a key={p.slug} className={`note-card newspaper-card ${index % 5 === 0 ? "wide" : ""}`} href={`#/notes/${p.slug}`} onClick={(e)=>{ e.preventDefault(); onPickPost && onPickPost(p.slug); }}>
            <div className="newspaper-illustration" style={{ "--note-ink": visual.ink, "--note-paper": visual.paper }}>
              <span>{visual.n}</span>
              <i />
              <b />
            </div>
            <div className="note-card-meta">{p.date}{p.tag ? <><span>·</span>{p.tag}</> : null}</div>
            <h2>{p.title}</h2>
            <p>{p.lede}</p>
          </a>
          );
    ...
    <div className="page note-page">
      ...
      <div className="note-reading-layout">
        <main className="note-main">
          <article className="article-shell note-article">
            <header className="note-hero newspaper-article-hero">
              <div>
                <div className="kicker"><span className="bullet" />{post.date}{post.tag ? ` · ${post.tag}` : ""}</div>
                <h1 className="display">{post.title}</h1>
                <p>{post.lede}</p>
              </div>
              <div className="newspaper-illustration article" style={{ "--note-ink": noteVisual(post, 0).ink, "--note-paper": noteVisual(post, 0).paper }}>
                <span>{(post.tag || "note").slice(0, 2).toUpperCase()}</span>
                <i />
                <b />
              </div>
...
window.Views = { CatalogueView, DetailView, ConcernsView, NutritionView, PlaybooksView, PlaybookDetailView, CompareView, ResearchView, SearchView, MethodologyView, AssessmentView, AccountView, PlansView, NotesView, NoteArticleView };
