  ...
  function apply() {
    ...
    if (visible.length === 0) {
      if (!empty) {
        ...
        empty.innerHTML = '<p>Aucune intervention ne correspond à ces filtres. Essayez d\'élargir la recherche ou de réactiver un niveau.</p>';
...
(function () {
  ...
  var TYPE_LABELS = { protocol: 'Protocole', post: 'Note', concern: 'Sujet' };
  ...
  function render(matches, q) {
    if (!matches.length) {
      box.innerHTML = '<div class="site-search-empty">Aucun résultat pour « ' + escHtml(q) + ' »</div>';
      box.hidden = false;
      return;
    }
