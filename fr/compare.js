...
(function () {
  ...
  function add(slug) {
    ...
    if (state.selected.length >= MAX) {
      alert(`Comparer plus de ${MAX} interventions à la fois devient illisible. Retirez-en une d'abord.`);
      return;
    }
  ...
  function shortCost(s) {
    ...
    if (sl.startsWith('gratuit') || sl.startsWith('free') || sl.includes('aucun coût') || sl.includes('économie')) return 'Gratuit';
    if (sl.includes('ordonnance uniquement') || sl.includes('not applicable')) return 'Sur ordonnance';
    const m = s.match(/(\d{1,4})\s*(?:[\-–à]+\s*(\d{1,4}))?\s*(€?\s*\/\s*(?:mois|jour|mo|month|day))?/);
    if (!m) return s.slice(0, 16);
    const lo = m[1], hi = m[2], unit = (m[3] || '').replace(/\s+/g, '').replace('month', 'mois').replace('mo', 'mois').replace('day', 'jour');
    return hi ? `${lo}–${hi} ${unit}` : `${lo} ${unit}`;
  }

  function tierLabel(t) {
    return ['—', 'Socle', 'Ciblé', 'Frontière'][t] || '—';
  }
  ...
  function renderColumn(slug) {
    ...
    if (!it) {
      return el('div', { class: 'cmp-col cmp-col-missing' },
        el('header', null,
          ...
          el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Retirer' }, '✕')
        ),
        el('p', null, 'Protocole introuvable.')
      );
    }
    ...
    return el('div', { class: `cmp-col card-tier-${tier}` },
      el('header', { class: 'cmp-col-head' },
        ...
        el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Retirer de la comparaison' }, '✕')
      ),
  ...
  function render() {
    ...
    const table = el('table', { class: 'cmp-table' },
      ...
      el('tbody', null,
        renderRow('Catégorie',        it => it.category || '—'),
        renderRow('Niveau',           it => tierLabel(it.tier || 2), { cellClass: 'cmp-cell-tier' }),
        renderRow('Efficacité',       it => `${it.effectiveness || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Certitude',        it => `${it.certainty || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Combiné (E×C)',    it => `${(it.effectiveness || 0) * (it.certainty || 0)}`, { cellClass: 'cmp-cell-num' }),
        renderRow('Niveau de preuves', it => it.evidence_level || '—'),
        renderRow('Coût',             it => shortCost(it.cost_signal)),
        renderRow('Verdict',          it => it.verdict || '—'),
        renderRow('Pour qui',         it => it.who_should_consider || '—'),
        renderRow('Déconseillé à',    it => it.who_should_skip || '—'),
        renderRow('Effets secondaires', it => {
          const arr = it.side_effects || [];
          return arr.length ? arr.slice(0, 5).join(' • ') : '—';
        }),
        renderRow('Contre-indications', it => {
        ...
        renderRow('Étiquettes', it => {
          const arr = it.tags || [];
          return arr.length ? arr.join(', ') : '—';
        }),
        renderRow('Ouvrir', it => {
          const a = el('a', { href: `/items/${it.slug}.html` }, 'Preuves complètes →');
          return a;
  ...
  function renderSearchResults(matches) {
    if (!matches.length) {
      results.innerHTML = '<div class="cmp-search-empty">Aucun résultat.</div>';
  ...
  fetch('/items.json', { cache: 'no-cache' })
    ...
    .catch(err => {
      board.innerHTML = '<p class="wiz-error">Échec du chargement des interventions : ' + err + '</p>';
    });
})();
