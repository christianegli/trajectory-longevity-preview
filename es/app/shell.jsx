...
const ROUTES = [
  ...
  { key: "playbooks", label: "Protocolos" },
  { key: "compare", label: "Comparar" },
  { key: "nutrition", label: "Nutrición" },
  { key: "concerns", label: "Por inquietud" },
  { key: "news", label: "Notas" },
  ...
  return (
    <header className="top">
      <div className="brand" onClick={() => setRoute("dashboard")} style={{ cursor: "pointer" }}>
        ...
        <span className="brand-tagline">Longevidad basada en la evidencia</span>
      </div>
      <nav className="nav">
        {ROUTES.map(r => (
      ...
      <div className="right">
        <div className="language-switcher" onBlur={(event) => {
          ...
          <button type="button" className="language-trigger" onClick={() => setLanguageOpen(open => !open)} aria-haspopup="menu" aria-expanded={languageOpen} title="Cambiar idioma">
            {activeLanguageLabel}
        ...
        <form className="search" onSubmit={runSearch}>
          ...
          <input
            ...
            placeholder={`Buscar entre ${itemCount} intervenciones`}
          />
          <span className="kbd">⌘K</span>
        </form>
        <button className="icon-btn" title="Notas" onClick={() => setRoute("news")} style={{ textDecoration: "none", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
