...
(function () {
  ...
  function add(slug) {
    ...
    if (state.selected.length >= MAX) {
      alert(`Comparing more than ${MAX} at once gets unreadable. Remove one first.`);
      return;
    }
  ...
  function shortCost(s) {
    ...
    if (sl.startsWith('free') || sl.includes('no cost') || sl.includes('massive savings')) return 'Free';
  ...
  function tierLabel(t) {
    return ['—', 'Foundation', 'Targeted', 'Frontier'][t] || '—';
  }
  ...
  function renderColumn(slug) {
    ...
    if (!it) {
      return el('div', { class: 'cmp-col cmp-col-missing' },
        el('header', null,
          ...
          el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Remove' }, '✕')
        ),
        el('p', null, 'Protocol not found.')
      );
    }
    ...
    return el('div', { class: `cmp-col card-tier-${tier}` },
      el('header', { class: 'cmp-col-head' },
        ...
        el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Remove from comparison' }, '✕')
      ),
  ...
  function render() {
    ...
    const table = el('table', { class: 'cmp-table' },
      ...
      el('tbody', null,
        renderRow('Category',         it => it.category || '—'),
        renderRow('Tier',             it => tierLabel(it.tier || 2), { cellClass: 'cmp-cell-tier' }),
        renderRow('Effectiveness',    it => `${it.effectiveness || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Certainty',        it => `${it.certainty || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Combined (E×C)',   it => `${(it.effectiveness || 0) * (it.certainty || 0)}`, { cellClass: 'cmp-cell-num' }),
        renderRow('Evidence level',   it => it.evidence_level || '—'),
        renderRow('Cost',             it => shortCost(it.cost_signal)),
        renderRow('Verdict',          it => it.verdict || '—'),
        renderRow('Worth considering if', it => it.who_should_consider || '—'),
        renderRow('Probably skip if',  it => it.who_should_skip || '—'),
        renderRow('Side effects', it => {
          const arr = it.side_effects || [];
          return arr.length ? arr.slice(0, 5).join(' • ') : '—';
        }),
        renderRow('Contraindications', it => {
          const arr = it.contraindications || [];
          return arr.length ? arr.slice(0, 5).join(' • ') : '—';
        }),
        renderRow('Targets', it => {
        ...
        renderRow('Open', it => {
          const a = el('a', { href: `/items/${it.slug}.html` }, 'Full evidence →');
          return a;
  ...
  function renderSearchResults(matches) {
    if (!matches.length) {
      results.innerHTML = '<div class="cmp-search-empty">No matches.</div>';
  ...
  fetch('/items.json', { cache: 'no-cache' })
    ...
    .catch(err => {
      board.innerHTML = '<p class="wiz-error">Failed to load items: ' + err + '</p>';
    });
})();
