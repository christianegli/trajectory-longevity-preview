  ...
  function apply() {
    ...
    if (visible.length === 0) {
      if (!empty) {
        ...
        empty.innerHTML = '<p>No interventions match these filters. Try broadening the search or re-enabling a tier.</p>';
...
(function () {
  ...
  var TYPE_LABELS = { protocol: 'Protocol', post: 'News', concern: 'Concern' };
  ...
  function render(matches, q) {
    if (!matches.length) {
      box.innerHTML = '<div class="site-search-empty">No matches for "' + escHtml(q) + '"</div>';
      box.hidden = false;
      return;
    }
