...
function FilterSidebar({ filters, setFilters, items }) {
  const Tags = ["Foundation","Cardio","Strength","Diet","Brain","Mind","Heat","Frontier","Senolytic","Prescription","OTC","NAD+","Mitophagy","Autophagy"];
  const toggle = (k, v) => {
    const cur = filters[k] || [];
  ...
  return (
    <aside style={{ width: 240, padding: "8px 16px 24px", borderRight: "1px solid var(--rule)", background: "var(--paper)", position: "sticky", top: 60, height: "calc(100vh - 60px)", overflowY: "auto", flexShrink: 0 }}>
      <div className="filter-block" style={{ borderBottom: 0, paddingBottom: 0 }}>
        <div className="filter-h"><span>Filters</span><button className="btn ghost" style={{ padding: "0 4px", fontSize: 10 }} onClick={() => setFilters({})}>Reset</button></div>
      </div>
      <div className="filter-block">
        <div className="filter-h">Tier</div>
      ...
      <div className="filter-block">
        <div className="filter-h">Evidence grade</div>
      ...
      <div className="filter-block">
        <div className="filter-h">Concerns</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div className="filter-block">
        <div className="filter-h">Tags</div>
...
function CatalogueView({ items, viz, onPick }) {
  ...
  const activeChips = [
    ...
    ...(filters.ev || []).map(t => ({ k: "ev", v: t, l: `Evidence ${t}` })),
    ...
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "100%" }}>
      ...
      <div style={{ padding: "24px 32px 80px", minWidth: 0 }}>
        <header style={{ marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 13.5, color: "var(--ink-2)" }}>{filtered.length} of {items.length} interventions · graded against 412 human studies</div>
        </header>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          {layout === "grid" && (
            <div style={{ display: "inline-flex", border: "1px solid var(--rule)", borderRadius: 8, overflow: "hidden", background: "var(--paper)" }}>
              {["comfy","dense"].map(d => (
                <button key={d} className="btn ghost" style={{ borderRadius: 0, padding: "7px 12px", fontSize: 12, background: density === d ? "var(--bg-2)" : "transparent" }} onClick={() => setDensity(d)}>{d}</button>
              ))}
            </div>
          )}
          <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: "7px 10px", border: "1px solid var(--rule)", borderRadius: 8, background: "var(--paper)", fontSize: 12 }}>
            <option value="effect">Sort: Effectiveness ↓</option>
        ...
        {layout === "list" && (
          <div className="card" style={{ padding: 0 }}>
            <div className="list-row" style={{ gridTemplateColumns: "auto 1.6fr 1fr 110px 90px 70px 24px", padding: "10px 18px", background: "var(--paper-2)", borderRadius: "14px 14px 0 0", cursor: "default" }}>
              ...
              <span className="mono" style={{ color: "var(--ink-3)" }}>Targets</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Score</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Evidence</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Cost</span>
              <span></span>
            </div>
            {filtered.map(item => (
        ...
        {layout === "scatter" && (
          <div className="card card-pad" style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 24, alignItems: "center" }}>
            ...
            <div>
              <div className="kicker" style={{ marginBottom: 8 }}>Reading the chart</div>
              <p style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.55, marginTop: 0 }}>
                Each dot is one intervention. Up-and-right is "big effect, very sure" — that's where you find sleep, exercise, statins. Down-and-right is the longevity frontier: maybe huge, maybe nothing.
              </p>
              <hr className="rule" style={{ margin: "12px 0" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="lifestyle" /> Lifestyle</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="supplement" /> Supplement</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="medication" /> Medication</div>
              </div>
            </div>
          </div>
        )}
