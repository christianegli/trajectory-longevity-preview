...
(function () {
  ...
  function add(slug) {
    ...
    if (state.selected.length >= MAX) {
      alert(`Mehr als ${MAX} gleichzeitig zu vergleichen wird unleserlich. Entfernen Sie zuerst eine Intervention.`);
      return;
    }
  ...
  function shortCost(s) {
    ...
    if (sl.startsWith('free') || sl.startsWith('kostenlos') || sl.includes('no cost') || sl.includes('massive savings')) return 'Gratis';
    if (sl.includes('not applicable') || sl.includes('rezeptpflichtig')) return 'Nur Rx';
  ...
  function tierLabel(t) {
    return ['—', 'Fundament', 'Gezielt', 'Frontier'][t] || '—';
  }
  ...
  function renderColumn(slug) {
    ...
    if (!it) {
      return el('div', { class: 'cmp-col cmp-col-missing' },
        el('header', null,
          ...
          el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Entfernen' }, '✕')
        ),
        el('p', null, 'Protokoll nicht gefunden.')
      );
    }
    ...
    return el('div', { class: `cmp-col card-tier-${tier}` },
      el('header', { class: 'cmp-col-head' },
        ...
        el('button', { type: 'button', class: 'cmp-remove', onclick: () => remove(slug), 'aria-label': 'Aus Vergleich entfernen' }, '✕')
      ),
  ...
  function render() {
    ...
    const table = el('table', { class: 'cmp-table' },
      ...
      el('tbody', null,
        renderRow('Kategorie',        it => it.category || '—'),
        renderRow('Stufe',            it => tierLabel(it.tier || 2), { cellClass: 'cmp-cell-tier' }),
        renderRow('Wirksamkeit',      it => `${it.effectiveness || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Gewissheit',       it => `${it.certainty || 0}/10`, { cellClass: 'cmp-cell-num' }),
        renderRow('Kombiniert (W×G)', it => `${(it.effectiveness || 0) * (it.certainty || 0)}`, { cellClass: 'cmp-cell-num' }),
        renderRow('Evidenz-Grad',     it => it.evidence_level || '—'),
        renderRow('Kosten',           it => shortCost(it.cost_signal)),
        renderRow('Fazit',            it => it.verdict || '—'),
        renderRow('Lohnt sich, wenn', it => it.who_should_consider || '—'),
        renderRow('Eher überspringen, wenn', it => it.who_should_skip || '—'),
        renderRow('Nebenwirkungen', it => {
          const arr = it.side_effects || [];
          return arr.length ? arr.slice(0, 5).join(' • ') : '—';
        }),
        renderRow('Kontraindikationen', it => {
        ...
        renderRow('Öffnen', it => {
          const a = el('a', { href: `/de/items/${it.slug}.html` }, 'Vollständige Evidenz →');
          return a;
  ...
  function renderSearchResults(matches) {
    if (!matches.length) {
      results.innerHTML = '<div class="cmp-search-empty">Keine Treffer.</div>';
    ...
    matches.forEach(it => {
      ...
      const row = el('button', {
        ...
        el('span', { class: 'cmp-search-meta' },
          ...
          el('span', null, `W ${it.effectiveness || 0} · G ${it.certainty || 0}`)
        )
      );
  ...
  fetch('/items.json', { cache: 'no-cache' })
    ...
    .catch(err => {
      board.innerHTML = '<p class="wiz-error">Konnte Daten nicht laden: ' + err + '</p>';
    });
})();
