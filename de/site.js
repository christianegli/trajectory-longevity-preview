  ...
  function apply() {
    ...
    if (visible.length === 0) {
      if (!empty) {
        ...
        empty.innerHTML = '<p>Keine Interventionen passen zu diesen Filtern. Versuchen Sie eine breitere Suche oder aktivieren Sie wieder eine Stufe.</p>';
...
(function () {
  ...
  var TYPE_LABELS = { protocol: 'Protokoll', post: 'Notiz', concern: 'Anliegen' };
  ...
  function render(matches, q) {
    if (!matches.length) {
      box.innerHTML = '<div class="site-search-empty">Keine Treffer für „' + escHtml(q) + '"</div>';
      box.hidden = false;
      return;
    }
