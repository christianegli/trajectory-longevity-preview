...
function FilterSidebar({ filters, setFilters, items }) {
  const Tags = ["Fundament","Cardio","Kraft","Ernährung","Gehirn","Geist","Hitze","Forschungsfront","Senolytika","Rezeptpflichtig","Rezeptfrei","NAD+","Mitophagie","Autophagie"];
  const toggle = (k, v) => {
    const cur = filters[k] || [];
  ...
  return (
    <aside style={{ width: 240, padding: "8px 16px 24px", borderRight: "1px solid var(--rule)", background: "var(--paper)", position: "sticky", top: 60, height: "calc(100vh - 60px)", overflowY: "auto", flexShrink: 0 }}>
      <div className="filter-block" style={{ borderBottom: 0, paddingBottom: 0 }}>
        <div className="filter-h"><span>Filter</span><button className="btn ghost" style={{ padding: "0 4px", fontSize: 10 }} onClick={() => setFilters({})}>Zurücksetzen</button></div>
      </div>
      <div className="filter-block">
        <div className="filter-h">Stufe</div>
      ...
      <div className="filter-block">
        <div className="filter-h">Kategorie</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[["lifestyle","Lebensstil"],["supplement","Supplemente"],["medication","Medikamente"]].map(([k,l]) => (
      ...
      <div className="filter-block">
        <div className="filter-h">Evidenzgrad</div>
      ...
      <div className="filter-block">
        <div className="filter-h">Anliegen</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div className="filter-block">
        <div className="filter-h">Schlagwörter</div>
...
function CatalogueView({ items, viz, onPick }) {
  ...
  const activeChips = [
    ...
    ...(filters.ev || []).map(t => ({ k: "ev", v: t, l: `Evidenz ${t}` })),
    ...
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "100%" }}>
      ...
      <div style={{ padding: "24px 32px 80px", minWidth: 0 }}>
        <header style={{ marginBottom: 20 }}>
          <div className="kicker" style={{ marginBottom: 6 }}>Bibliothek</div>
          <h1 style={{ fontFamily: "var(--display)", fontStyle: "var(--display-italic)", fontWeight: "var(--display-w)", fontSize: 32, margin: "0 0 4px", letterSpacing: "-0.02em" }}>Der Katalog</h1>
          <div style={{ fontSize: 13.5, color: "var(--ink-2)" }}>{filtered.length} von {items.length} Interventionen · bewertet gegen 412 Humanstudien</div>
        </header>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ display: "inline-flex", border: "1px solid var(--rule)", borderRadius: 8, overflow: "hidden", background: "var(--paper)" }}>
            {["grid","list","scatter"].map(l => (
              <button key={l} className="btn ghost" style={{ borderRadius: 0, padding: "7px 12px", fontSize: 12, background: layout === l ? "var(--bg-2)" : "transparent" }} onClick={() => setLayout(l)}>
                {{grid:"Raster",list:"Liste",scatter:"Streudiagramm"}[l]}
              </button>
            ))}
          </div>
          {layout === "grid" && (
            <div style={{ display: "inline-flex", border: "1px solid var(--rule)", borderRadius: 8, overflow: "hidden", background: "var(--paper)" }}>
              {["comfy","dense"].map(d => (
                <button key={d} className="btn ghost" style={{ borderRadius: 0, padding: "7px 12px", fontSize: 12, background: density === d ? "var(--bg-2)" : "transparent" }} onClick={() => setDensity(d)}>{{comfy:"Komfort",dense:"Dicht"}[d]}</button>
              ))}
            </div>
          )}
          <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: "7px 10px", border: "1px solid var(--rule)", borderRadius: 8, background: "var(--paper)", fontSize: 12 }}>
            <option value="effect">Sortieren: Wirksamkeit ↓</option>
            <option value="certainty">Sortieren: Gewissheit ↓</option>
            <option value="combined">Sortieren: Kombiniert ↓</option>
            <option value="alpha">Sortieren: A–Z</option>
          </select>
        ...
        {layout === "list" && (
          <div className="card" style={{ padding: 0 }}>
            <div className="list-row" style={{ gridTemplateColumns: "auto 1.6fr 1fr 110px 90px 70px 24px", padding: "10px 18px", background: "var(--paper-2)", borderRadius: "14px 14px 0 0", cursor: "default" }}>
              <span></span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Intervention</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Ziele</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Score</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Evidenz</span>
              <span className="mono" style={{ color: "var(--ink-3)" }}>Kosten</span>
              <span></span>
            </div>
            {filtered.map(item => (
        ...
        {layout === "scatter" && (
          <div className="card card-pad" style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 24, alignItems: "center" }}>
            ...
            <div>
              <div className="kicker" style={{ marginBottom: 8 }}>Diagramm lesen</div>
              <p style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.55, marginTop: 0 }}>
                Jeder Punkt ist eine Intervention. Oben rechts: große Wirkung, hohe Gewissheit — dort finden Sie Schlaf, Bewegung, Statine. Unten rechts liegt die Longevity-Forschungsfront: kann gewaltig sein, kann auch nichts sein.
              </p>
              <hr className="rule" style={{ margin: "12px 0" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="lifestyle" /> Lebensstil</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="supplement" /> Supplemente</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CDot cat="medication" /> Medikamente</div>
              </div>
            </div>
          </div>
        )}
